...
class EmprestimoWindow(QMainWindow):
    def __init__(self, stacked_widget, db_manager):
        super(EmprestimoWindow, self).__init__()
        ...
        # Carregar o arquivo emprestimo.ui da pasta show
        ui_path = os.path.join(os.path.dirname(__file__), "emprestimo.ui")
        loadUi(ui_path, self)
        ...
        if self.menuButton:
            ...
            print("[ERRO] menuButton não encontrado no arquivo emprestimo.ui")

        # Referência ao QTableWidget para exibir os dados da tabela emprestimo
        self.tabela: QTableWidget = self.findChild(QTableWidget, "tabela")
        if not self.tabela:
            print("[ERRO] tabela não encontrada no arquivo emprestimo.ui")

        # Referência ao QLabel para exibir a quantidade de empréstimos
        self.quantidadeInput: QLabel = self.findChild(QLabel, "quantidadeInput")
        if not self.quantidadeInput:
            print("[ERRO] quantidadeInput não encontrado no arquivo emprestimo.ui")

        # Carregar os dados da tabela emprestimo e a quantidade de empréstimos
        self.load_emprestimos()
        self.load_quantidade()

    def showEvent(self, event):
        """Atualiza a tabela e a quantidade de empréstimos sempre que a janela for exibida."""
        self.load_emprestimos()
        self.load_quantidade()
        super().showEvent(event)

    def load_emprestimos(self):
        """Carrega os dados da tabela emprestimo e exibe no QTableWidget."""
        if not self.tabela:
            print("[ERRO] tabela não está inicializada.")
            return

        try:
            # Consulta ao banco de dados para obter os dados da tabela emprestimo
            query = "SELECT * FROM emprestimo"
            results = self.db_manager.fetch_query(query)

            if not results:
                print("[INFO] Nenhum dado encontrado na tabela emprestimo.")
            ...
            print(f"[ERRO] Não foi possível carregar os dados da tabela emprestimo: {e}")

    def load_quantidade(self):
        """Carrega a quantidade de empréstimos e exibe no QLabel."""
        if not self.quantidadeInput:
            print("[ERRO] quantidadeInput não está inicializado.")
            return

        try:
            # Consulta ao banco de dados para contar o número de empréstimos
            query = "SELECT COUNT(*) AS quantidade FROM emprestimo"
            ...
            if result:
                ...
                self.quantidadeInput.setText("0")  # Exibe 0 se não houver empréstimos

        except Exception as e:
            print(f"[ERRO] Não foi possível carregar a quantidade de empréstimos: {e}")
